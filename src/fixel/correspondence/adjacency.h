...
#ifndef __fixel_correspondence_adjacency_h__
#define __fixel_correspondence_adjacency_h__
    ...
    namespace Correspondence {
      ...
      { MEMALIGN(Adjacency)
        ...
        public:
          ...
          {
            if (fixels.size() < FIXELCORRESPONDENCE_MIN_DIRS_TO_ENFORCE_ADJACENCY)
              return;
          ...
          bool operator() (const vector<uint32_t>& indices) const
          {
            ...
            for (const auto i : indices) {
              ...
              if (std::none_of (list.begin(), list.end(), [&] (const DWI::Directions::index_type j) { return std::find (indices.begin(), indices.end(), j) != indices.end(); }))
                return false;
            }
#endif            
            return true;
