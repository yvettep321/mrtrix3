                                                 ...
                                                 const Eigen::Array<int8_t, Eigen::Dynamic, 1>& origins_per_remapped_fixel)
            {
              ...
              for (uint32_t index = 0; index != rs.size(); ++index) {
                if (rs[index].density()) {
                  ...
                  // const float angle = std::acos (t[index].absdot(rs[index]));
                  // const float orientation_error_term = std::tan (angle);
                  // result += Math::pow2 (t[index].density() - rs[index].density()) * orientation_error_term;
