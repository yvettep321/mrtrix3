...
#define FIXELCORRESPONDENCE_MAX_FIXELS_FOR_COMBINATORIAL_WARNING 5
      ...
      namespace Algorithms {
        ...
        { NOMEMALIGN
          ...
          public:
            ...
            {
              if (std::max (s.size(), t.size()) > FIXELCORRESPONDENCE_MAX_FIXELS_FOR_COMBINATORIAL_WARNING
                ...
                WARN("Excessive fixel counts can currently lead to prohibitively long execution times; "
                     "suggest limiting maximal fixel count per voxel to no greater than " + str(FIXELCORRESPONDENCE_MAX_FIXELS_FOR_COMBINATORIAL_WARNING));
              }
              ...
              for (uint32_t r = s.size(); r > max_origins_per_target; --r)
                permissible_src_fixel_combinations -= Math::binomial (s.size(), r);
#endif
              ...
              if (s.size() < FIXELCORRESPONDENCE_MIN_DIRS_TO_ENFORCE_ADJACENCY)
